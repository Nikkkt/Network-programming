...
class Program
{
    ...
    static string ProcessRequest(string request)
    {
        ...
        if (partsPrices.ContainsKey(request.ToLower())) return "Price for " + request + ": $" + partsPrices[request.ToLower()];
        else return "Spare part \"" + request + "\" not found";
    }

    static bool CheckRequestLimit(IPAddress clientIP)
    {
        if (!clientRequests.ContainsKey(clientIP)) clientRequests.Add(clientIP, new Queue<DateTime>());

        var requests = clientRequests[clientIP];

        while (requests.Count > 0 && (DateTime.Now - requests.Peek()).TotalHours >= 1) requests.Dequeue();

        if (requests.Count < 10) { requests.Enqueue(DateTime.Now); return true; }
        else return false;
    }
}